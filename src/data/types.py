...
class SolverVerdict(BaseModel):
    ...
    answer: str | None
    status: Literal[
        "solved", "unknown", "timeout", "parse_error", "skipped", "inconsistent"
    ]
    unsat_core: list[str] = Field(default_factory=list)
    # For an "Uncertain" verdict there is no unsat core to point at. Instead we
    # capture a counter-model witness: two consistent scenarios (one making the
    # goal true, one making it false) over the ground atoms the problem mentions,
    # which is the evidence that the goal is genuinely undetermined.
